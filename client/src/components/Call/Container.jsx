...
function Container({data}) {
  ...
  useEffect(() => {
    const startCall = async () => {
      import("zego-express-engine-webrtc").then(async({ZegoExpressEngine}) => {
        const zg = new ZegoExpressEngine(process.env.ZEGO_APP_ID,process.env.ZEGO_SERVER_SECRET)
        setZgVar(zg)
  ...
  return (
    <div className="border-l border-conversation-border flex flex-col bg-conversation-panel-background w-full h-[100vh] overflow-hidden items-center justify-center text-white">
      <div className="flex flex-col gap-3 items-center">
        <span className="text-5xl">{data.name}</span>
        <span className="text-lg">
          {callAccepted && data.callType !== "video" ? "Ongoing Call" : "Calling"}
        </span>
      </div>
      ...
      <div className="h-16 rounded-full flex items-center justify-center bg-red-600 w-16">
